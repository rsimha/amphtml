 ...
 *   from the `main` branch, until reaching `main` or the first commit that
 *   was added directly on this branch (if the current commit is on `main`'s
 *   commit history, or only contains new commits that are not cherry-picked
 *   from `main`, then <X> is 0).
 * - Find the commit (<C>) before the last cherry-picked commit from the
 *   `main` branch (if the current branch is `main`, or otherwise in
 *   `main`'s commit history, then the current commit is <C>).
 ...
 *   was merged into `main`, or a commit was cherry-picked onto the branch;
 ...
 *
 * Examples:
 * 1. The version number of a release built from the HEAD commit on `main`,
 ...
 *    - The last three digits are 000 as this commit is on `main`.
 *
 * 2. The version number of a release built from a local working branch (e.g.,
 *    on a developer's workstation) that was split off from a `main` commit
 ...
 *      split from `main`, and does not have any cherry-picked commits.
 *
 * 3. For a release built from a local working branch that was split off from a
 *    `main` commit from November 9, 2021 11:48 PM PST, and then:
 *    - had one commit that was cherry-picked from `main`,
 ...
 *    - followed by twelve commits that were cherry-picked from `main`, then
 *      its version number would be `2111240238012`.
 *    - The latest twelve commits are cherry-picks from `main`, and the one
...
function getVersion() {
  ...
  const commitCherriesInfo = gitCherryMain().reverse();
